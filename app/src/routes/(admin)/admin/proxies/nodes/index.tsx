...
// A node is considered online if it posted a heartbeat within this window.
...
function isOnline(node: Node): boolean {
  ...
  return Date.now() - new Date(node.lastSeenAt).getTime() < ONLINE_THRESHOLD_MS;
}
...
function RouteComponent(): React.ReactElement {
  ...
  const regenMutation = useMutation({
    mutationFn: (id: string) => regenerateAgentToken({ data: { id } }),
  ...
  const requestDelete = async (node: Node) => {
  ...
  // The secret token is never stored, so this copies only non-secret connection
  // info (id, endpoint, token prefix). The token itself is shown once on issue.
  const copyAgentInfo = async (node: Node) => {
    const info = JSON.stringify(
      {
        nodeId: node.id,
        endpoint: `${node.address}:${node.listenPort}`,
        tokenPrefix: node.agentTokenPrefix,
      },
      null,
      2,
    );
    ...
    toastManager.add({
      type: "success",
      title: m.admin_proxies_nodes_toast_copied(),
    });
  };
          ...
          <div className="flex justify-end">
            <Menu>
              ...
              <MenuPopup align="end">
                ...
                <MenuItem onClick={() => void copyAgentInfo(node)}>
                  {m.admin_proxies_nodes_action_copy_agent()}
                </MenuItem>
                <MenuItem onClick={() => regenMutation.mutate(node.id)}>
                  {m.admin_proxies_nodes_action_reset_token()}
                </MenuItem>
                <MenuSeparator />
                <MenuItem
                  variant="destructive"
